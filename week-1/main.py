...
@app.put("/task/{id}", response_model= TaskResponse, status_code=status.HTTP_202_ACCEPTED)
def update_record(id: int, task: TaskRequest):
    task_to_update = next(filter(lambda task: task.task_id == id, tasks),
    None)
    if task_to_update is None:
        raise HTTPException()

    task_to_update.task_id = id
...
@app.patch("/task/{id}", response_model=TaskResponse, status_code=status.HTTP_200_OK)
def update_specific_record(id:int, task: TaskUpdate):
    task_to_update = next(filter(lambda task: task.task_id == id, tasks),
    None)
    if task.task_id is not None:
        task_to_update.task_id = task.task_id
